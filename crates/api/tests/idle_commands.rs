...
async fn test_idle_periods_save_and_retrieve() {
    ...
    // Save idle period
    ctx.idle_periods
        .save_idle_period(period.clone())
        .await
        .expect("save_idle_period failed");
...
async fn test_idle_periods_multiple_results_sorted() {
    ...
    ctx.idle_periods
        .save_idle_period(period1.clone())
        .await
        .expect("save period1 failed");
    ctx.idle_periods
        .save_idle_period(period2.clone())
        .await
        .expect("save period2 failed");
    ctx.idle_periods
        .save_idle_period(period3.clone())
        .await
        .expect("save period3 failed");
...
async fn test_get_pending_idle_periods() {
    ...
    ctx.idle_periods
        .save_idle_period(pending.clone())
        .await
        .expect("save pending failed");
    ctx.idle_periods
        .save_idle_period(kept.clone())
        .await
        .expect("save kept failed");
    ctx.idle_periods
        .save_idle_period(pending2.clone())
        .await
        .expect("save pending2 failed");

    // Get pending periods
    let pending_periods = ctx
        .idle_periods
        .get_pending_idle_periods()
        .await
        .expect("get_pending_idle_periods failed");

    assert_eq!(
        pending_periods.len(),
        2,
        "expected 2 pending periods (NULL and 'pending')"
    );
...
async fn test_get_idle_summary_empty_day() {
    ...
    let summary = ctx
        .idle_periods
        .get_idle_summary(now - 3600, now)
        .await
        .expect("get_idle_summary failed");
...
async fn test_get_idle_summary_with_data() {
    ...
    ctx.idle_periods
        .save_idle_period(idle1.clone())
        .await
        .expect("save idle1 failed");
    ctx.idle_periods
        .save_idle_period(idle2.clone())
        .await
        .expect("save idle2 failed");
    ctx.idle_periods
        .save_idle_period(idle3.clone())
        .await
        .expect("save idle3 failed");
...
async fn test_idle_sync_metrics_record_activity_wake() {
    ...
    // Record wake events
    ctx.idle_sync_metrics
        .record_activity_wake("mouse".to_string());
    ctx.idle_sync_metrics
        .record_activity_wake("keyboard".to_string());
...
async fn test_idle_sync_metrics_record_timer_events() {
    ...
    ctx.idle_sync_metrics
        .record_timer_event_emission(1000, true);
    ctx.idle_sync_metrics
        .record_timer_event_emission(2000, false);
...
async fn test_idle_sync_metrics_record_state_transition() {
    ...
    ctx.idle_sync_metrics
        .record_state_transition("idle", "active", 100);
    ctx.idle_sync_metrics
        .record_state_transition("active", "idle", 50);
